...
# method = "forward":
regfit.fwd <- regsubsets(MENTHLTH~.-arthritis, data = brfss.train, nvmax = 10, 
                         method ="forward")
...
coefi <- coef(regfit.fwd, id = 5)


# method = "backward":
regfit.bwd = regsubsets(MENTHLTH~.-arthritis, data = brfss.train, nvmax = 10, 
                        method = "backward")
(bwd.summary <- summary(regfit.bwd))
# Now use our new plot function:
plot.regsummary(bwd.summary)

test.mat <- model.matrix(MENTHLTH~.-arthritis, data = brfss.test, nvmax = 10, 
                         method = "backward")
val.errors <- rep(NA, 10)
for (i in 1:10){
